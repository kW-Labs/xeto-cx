...
```
cx/                 → root vocabulary (phases, roles, disciplines, planes, modes, CxProject, tag vocabulary)
cx.requirements/    → OPR/BOD contract layer + traceability spine
...
- Family libs additionally depend on `cx` (never on each other, except `cx.content` which depends on `cx`, `cx.requirements`, `cx.verification`)
...
- **Versions stay `0.1.0`** until the first real-project validation completes.
